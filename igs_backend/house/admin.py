...
class HouseAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        """
        Override to exclude soft-deleted houses from appearing in the admin panel.
        """
        queryset = super().get_queryset(request)
        return queryset.filter(is_deleted=False)  # Exclude soft-deleted houses
    ...
    def view_details(self, obj):
        return format_html(
            '<a href="/admin/house/house/{}/change/" class="button" '
