...
from account.serializers import RequestAccountSerializer, ResponseAccountSerializer
...
class AccountViewSet(viewsets.ModelViewSet):
    def get_serializer_class(self):
        return RequestAccountSerializer

    def get_permissions(self):
        """
        Custom method to define permissions for each action.
        """
        agent_landlord_actions = {'accounts'}
    ...
    @swagger_auto_schema(
        ...
        method="get",
        tags=["Account"],
        responses={200: ResponseAccountSerializer(many=True), 400: "Invalid input data"},
    )
    ...
    def account(self, request: HttpRequest):
        ...
        if agent is None and landlord is None:
            return Response(data={"detail": "You are forbidden to view this information"}, status=status.HTTP_403_FORBIDDEN)

        try:
            account = Account.get_account(agent=agent, landlord=landlord)
            response_serializer = ResponseAccountSerializer(account, many=True)
